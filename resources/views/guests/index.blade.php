...
@extends('layouts.app')  {{-- este es tu layout principal del dashboard --}}
...
@section('content')
    <div class="max-w-6xl mx-auto">
        ...
        @if (session('import_summary'))
            ...
            <div class="mb-4 p-3 rounded-md bg-blue-50 text-blue-800 text-sm">
        ...
        <div class="bg-white rounded-lg shadow-sm p-4 mb-8">
            <h2 class="text-lg font-semibold mb-3">Agregar invitado</h2>
            ...
            <form action="{{ route('guests.store') }}" method="POST"
                ...
                <div>
                    ...
                    <input type="text" name="full_name" class="w-full border-gray-300 rounded-md text-sm" required>
                </div>

                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">Email</label>
                    <input type="email" name="email" class="w-full border-gray-300 rounded-md text-sm">
                </div>

                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">Teléfono</label>
                    <input type="text" name="phone" class="w-full border-gray-300 rounded-md text-sm">
                </div>
        ...
        {{-- Bloque de importación masiva (4 filas) --}}
    ...
    {{-- Input de archivo --}}
    <label for="file_csv"
           class="inline-flex items-center justify-center px-4 py-2 border border-dashed border-purple-300 rounded-md text-sm text-purple-700 bg-purple-50 hover:bg-purple-100 cursor-pointer">
        📎 Seleccionar archivo
    ...
    {{-- Botón de importar --}}
    <button type="submit"
            class="inline-flex items-center justify-center px-4 py-2 rounded-md text-sm font-semibold text-white bg-purple-600 hover:bg-purple-700">
        📥 Importar invitados
    </button>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tr class="border-b text-left text-xs uppercase text-gray-500">
                        ...
                        <th class="py-2 pr-4">Acompañantes</th>
                    ...
                    @forelse ($guests as $guest)
                        <tr class="border-b last:border-0">
                            <td class="py-2 pr-4">{{ $guest->full_name }}</td>
                            <td class="py-2 pr-4">{{ $guest->email ?? '-' }}</td>
                            <td class="py-2 pr-4">{{ $guest->phone ?? '-' }}</td>
                            <td class="py-2 pr-4">{{ $guest->max_companions }}</td>
                            <td class="py-2 pr-4">
                                @switch($guest->status)
                                    @case('confirmed')
